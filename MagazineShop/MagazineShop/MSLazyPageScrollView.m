...
#define horizontalWorflow(x, y)                 ((_flow == MSLazyPageScrollViewFlowHorizontal) ? x : y)
...
@interface MSLazyPageScrollView () <UIScrollViewDelegate>

@property (nonatomic, assign) CGSize internalPageSize;
@property (nonatomic) MSLazyPageScrollViewFlow flow;

- (void)_updateUIForCurrentOffset;
- (MSLazyPageView *)_viewForIndex:(NSInteger)index;
...
@implementation MSLazyPageScrollView
...
- (void)_updateUIForCurrentOffset
{
	...
	for (int i = minVisibleIndex; i <= maxVisibleIndex; i++) {
		MSLazyPageView *existingView = nil;
		for (MSLazyPageView *pageView in _visibleViews) {
			if (pageView.index == i) {
		...
		if (existingView) {
			...
			MSLazyPageView *newView = [self _viewForIndex:i];
...
- (MSLazyPageView *)_viewForIndex:(NSInteger)index
{
	MSLazyPageView *finalPage = nil;
	
	if (_pageScrollViewFlags.dataSourcePagesOnly) {
		finalPage = [_dataSource pageScrollView:self pageViewAtIndex:index];
	} else {
		MSLazyPageView *pageViewContainer = [self dequeuePageForIdentifer:FT2PageContainerIdentifier];
		if (pageViewContainer == nil) {
			pageViewContainer = [[MSLazyPageView alloc] initWithReuseIdentifier:FT2PageContainerIdentifier];
		...
		if ([reusedView respondsToSelector:@selector(prepareForReuse)]) {
			[(id <MSLazyReusableView>)reusedView prepareForReuse];
		}
	...
	if (_flow == MSLazyPageScrollViewFlowHorizontal) {
	...
	for (MSLazyPageView *pageView in _visibleViews) {
		...
		if (pageView.usedAsContainer) {
			...
			if ([contentView respondsToSelector:@selector(willBeDiscarded)]) {
				[(id <MSLazyReusableView>)contentView willBeDiscarded];
			}
...
- (id)dequeuePageForIdentifer:(NSString *)identifier
{
	...
	if (set) {
		MSLazyPageView *page = [set anyObject];
		if (page) {
...
{
	for (MSLazyPageView *page in _visibleViews) {
...
- (NSArray *)visibleViews
{
	...
	for (MSLazyPageView *pageView in sortedPageViews) {
...
{
	self = [self initWithFrame:frame contentFlow:MSLazyPageScrollViewFlowHorizontal];
	return self;
}
...
- (UIView *)selectedView
{
	...
	for (MSLazyPageView *v in _visibleViews) {
...
- (id <MSLazyPageScrollViewDelegate>)delegate
{
...
- (void)setDataSource:(id <MSLazyPageScrollViewDataSource>)d
{
	_dataSource = d;
...
- (void)setDelegate:(id<MSLazyPageScrollViewDelegate>)d
{
